...
export const CartProvider = ({children}) => {
  ...
  const totalToPay = () => {
    ...
    const priceQuantity = productsCart.map(
  ...
  const isInCart = (idProduct) =>
    productsCart.some((product) => product.id === idProduct);

  // función para AÑADIR al carrito
  const addToCart = (productObject) =>
    !isInCart(productObject.id) && productsCart.push(productObject);

  // función para EDITAR cantidad
  const addQuantity = (idProduct, quantity) =>
    productsCart.map((b) =>
      b.id === idProduct && quantity > 0 ? (b.quantity = quantity) : null
    );
